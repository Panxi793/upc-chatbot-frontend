 ...
 */
export async function uploadToS3(file: File, folder: string): Promise<UploadResponse> {
    try {
        if (!file) {
        ...
        const response = await fetch('/api/files', {
            method: 'POST',
            body: JSON.stringify({
                // Pass the folder path directly to the API
                // The API will handle prepending app-images/ if needed
                folder: folder,
                fileName: file.name,
                fileSize: file.size,
...
export async function uploadImageToS3(
    ...
    try {
        ...
        return uploadToS3(resizedFile, folder);
    } catch (error) {
